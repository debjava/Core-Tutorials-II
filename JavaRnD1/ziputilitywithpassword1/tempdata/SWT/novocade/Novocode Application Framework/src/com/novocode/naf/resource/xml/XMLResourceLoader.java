...
public final class XMLResourceLoader implements ResourceLoader
{
  ...
  public XMLResourceLoader(ResourceManager resourceManager) throws NAFException
  {
    ...
    catch(Exception ex) { throw new NAFException("Error creating XMLResourceLoader", ex); }
  }
  ...
  private void addDirectComponent(ConfPropertyDescriptor pd, Element e, XMLResource resource, ConfigurableObject target) throws NAFException
  {
    ...
    if(facClass != null)
    {
      ...
      if(ch instanceof ConfigurableObject)
        initObject(((ConfigurableObject)ch), e, resource, target, false);
    }
    else
    {
      if(pd.isMap())
  ...
  private <T> T createDirectComponent(Class<T> clazz, Element e, XMLResource resource, ConfigurableObject parent, boolean ignoreUnknownElements) throws NAFException
  {
    ...
    if(co instanceof ConfigurableObject)
      initObject(((ConfigurableObject)co), e, resource, parent, ignoreUnknownElements);
    return co;
  }
  ...
  private void initObject(ConfigurableObject o, Element thisEl, XMLResource resource, ConfigurableObject parent, boolean ignoreUnknownElements) throws NAFException
  {
    ...
    for(Node n = thisEl.getFirstChild(); n != null; n = n.getNextSibling())
    {
      if(n.getNodeType() == Node.ELEMENT_NODE && XMLResourceLoader.NAF_NAMESPACE_URI.equals(n.getNamespaceURI()))
      {
        ...
        if(ln.equals("import"))
          resource.addImport(new Import(e));
        else if((pd = propertyManager.getElementPropertyDescriptor(ln)) != null)
          addDirectComponent(pd, e, resource, o);
        else
        {
          if((pd = propertyManager.getNestedElementsPropertyDescriptor()) != null)
            pd.addCompound(o, createComponent(e, resource, o));
          else if(!ignoreUnknownElements)
            throw new NAFException("Unrecognized child element "+e.getLocalName()+" in "+o.getClass().getName());
        }
      }
      else if(n.getNodeType() == Node.TEXT_NODE)
      {
        if((pd = propertyManager.getAttributePropertyDescriptor(":text", '.')) != null)
          pd.addCompound(o, ((Text)n).getData());
      }
    }
