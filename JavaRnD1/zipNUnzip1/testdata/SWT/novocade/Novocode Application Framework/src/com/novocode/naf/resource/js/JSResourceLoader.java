...
public class JSResourceLoader implements ResourceLoader
{
  ...
  public JSResourceLoader()
  {
    ...
    try
    {
      ...
      //scope.put("Window", scope, new Component.ComponentCreator("Window", scope));
      //scope.put("Shell", scope, new Component.ComponentCreator("Shell", scope));
      ...
      //String s = "myCounter.count; myCounter.count;";
      //String s = "Packages.com.novocode.naf.js.example.test.Test.log('foo'); out.println(42);";
      //Object result = cx.evaluateString(scope, s, "<cmd>", 1, null);
      
      /*Reader in = new InputStreamReader(Test.class.getResourceAsStream("hello.js"), "UTF-8");
      try
      {
        Object result = cx.evaluateReader(scope, in, "hello.js", 1, null);
        System.out.println("RESULT: "+Context.toString(result));
      }
      finally { in.close(); }*/

      /*Object x = scope.get("x", scope);
      if(x == Scriptable.NOT_FOUND) System.out.println("x is not defined.");
      else System.out.println("x = " + Context.toString(x));

      Object fObj = scope.get("f", scope);
      if(!(fObj instanceof Function)) System.out.println("f is undefined or not a function.");
      else
      {
    ...
    catch(Exception ex) { throw new NAFException("Error building JS scope", ex); }
  ...
  void nafDefine(Object spec) throws NAFException
  {
    ...
    if(resource == null)
      throw new NAFException("define() must only be called synchronously from within readResource()");
    ...
    if(!(spec instanceof ScriptableObject))
      throw new NAFException("define() must be called on a JavaScript object");
  ...
  void nafRunMainWindow(Object spec) throws NAFException
  {
    Resource resource = threadResource.get();
    if(resource == null)
      throw new NAFException("runMainWindow() must only be called synchronously from within readResource()");
    if(app == null)
      throw new NAFException("JSResourceLoader needs an associated NAFApplication to perform application functions");
    ...
    if(!(spec instanceof ScriptableObject))
      throw new NAFException("define() must be called on a JavaScript object");
    Object o = createComponent((ScriptableObject)spec, resource, null);
    if(!(o instanceof IWindowInstanceWidget))
      throw new NAFException("NAF.runMainWindow requires an IWindowInstanceWidget");
  ...
  private Class<?> findClassFor(ScriptableObject spec)
  {
    ...
    {
      try { clazz = Class.forName((String)cls); }
      catch(Exception ex) { throw new NAFException("Error finding named class "+cls, ex); }
    }
    else if(cls instanceof NativeJavaClass) clazz = ((NativeJavaClass)cls).getClassObject();
    else throw new NAFException("Object has no \"$\" property to determine the class to instantiate: "+spec);
    return clazz;
  }
  ...
  {
    Object cls = spec.get("$", nafObject);
    if(cls instanceof String) return (String)cls;
    else return null;
  }
  ...
  private void addDirectComponent(ConfPropertyDescriptor pd, ScriptableObject spec, Resource resource, ConfigurableObject target) throws NAFException
  {
    ...
    if(facClass != null)
    {
      ...
      if(ch instanceof ConfigurableObject)
        initObject(((ConfigurableObject)ch), spec, resource, target, false);
    }
    else
    {
      if(pd.isMap())
  ...
  {
    T co;
    try { co = clazz.newInstance(); }
    catch(Exception ex) { throw new NAFException("Error instantiating element object from class "+clazz.getName(), ex); }

    if(co instanceof ConfigurableObject)
      initObject(((ConfigurableObject)co), spec, resource, parent, ignoreUnknownElements);
    return co;
  }
  ...
  private void addChild(ConfigurableObject o, Resource resource, ConfPropertyManager propertyManager, Object val, boolean ignoreUnknownElements)
  {
    ...
    if(val instanceof NativeJavaClass)
    {
      ...
      else if(!ignoreUnknownElements)
        throw new NAFException("Unrecognized child object [class "+ncls.getName()+"] in "+o.getClass().getName());
    }
    else if(val instanceof ScriptableObject)
    {
      String chcls = findClassNameFor((ScriptableObject)val);
      if((pd = propertyManager.getElementPropertyDescriptor(chcls)) != null)
        addDirectComponent(pd, (ScriptableObject)val, resource, o);
      else
      {
        //--
        if((pd = propertyManager.getNestedElementsPropertyDescriptor()) != null)
        {
          ...
          throw new NAFException("Unrecognized child object "+chcls+" in "+o.getClass().getName());
      }
      //--
    }
    else if(val instanceof String)
    {
      if((pd = propertyManager.getAttributePropertyDescriptor(":text", '$')) != null)
        pd.addCompound(o, val);
    }
  ...
  private void initObject(ConfigurableObject o, ScriptableObject spec, Resource resource, ConfigurableObject parent, boolean ignoreUnknownElements) throws NAFException
  {
    ...
    for(Object id : spec.getIds())
    {
      ...
      {
        if(pd.isCompound()) // Prefix match
        {
          Object map = pd.getValue(o);
          Object jval = val;
          if(pd.getInstanceType() == ModelBinding.class) // Special support for static model bindings
          {
        ...
        else
        {
          ...
          //[TODO] Support more direct conversions
          if(val == null || val instanceof String) pd.setSimple(o, (String)val);
          else if(propType == Integer.TYPE || propType == Integer.class)
          {
            if(val instanceof Integer) pd.setValue(o, val);
            if(val instanceof Number) pd.setValue(o, ((Number)val).intValue());
          ...
          else if(propType == Double.TYPE || propType == Double.class)
          {
            ...
            if(val instanceof Number) pd.setValue(o, ((Number)val).doubleValue());
            else pd.setSimple(o, val.toString());
          }
          else if(propType == Boolean.TYPE || propType == Boolean.class)
          {
            if(val instanceof Boolean) pd.setValue(o, val);
            else pd.setSimple(o, val.toString());
          }
          else if(propType == BoundModelFactory.class) // Special support for static model bindings
          {
